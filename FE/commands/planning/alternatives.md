...
각 대안에 대해:
1. 구현 방법 (React/CRA/JSX 기준, page/hook/API client/guard 레이어 수준)
2. 기존 모듈(module-registry.yaml)과의 호환성
3. 기술적 위험 요소와 복잡도 (1~5)
   - main thread blocking 가능성
   - 메모리 사용량
   - 동시성/race condition
4. 의존성 추가 필요 여부 (npm 패키지, 외부 서비스)
5. API 계약 변경 / API 계약 변경 위험도 (low/medium/high)
6. 브라우저 이벤트 스키마 변경 필요 여부
...
각 대안에 대해:
1. 사용자(프론트엔드 사용자/작성자/모더레이터)별 경험 변화
2. 즉시성/응답성 차이 (즉시 반영 vs 지연 vs polling)
...
2. Azure 인프라 비용 변동 (cache 메모리, Swarm replica 추가, 트래픽 증가)
...
- 복잡도: {1~5}
- 의존성: ...
- API 계약 또는 환경 설정 위험: {low|medium|high}
- 브라우저 이벤트/반응형 변경: {있음|없음}
- BE 연동 변경: {있음|없음}
