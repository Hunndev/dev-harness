...
3. sub-agent는 다음을 조사한다:
   - 유사 서비스/기능의 구현 사례 (프론트엔드/모바일 웹/알림/커뮤니티 UI)
   - 관련 npm 패키지, route guard, 브라우저 이벤트/반응형 패턴
   - 알려진 함정이나 안티패턴 (중복 API 호출, main thread blocking, memory leak, race condition)
   - BUCCL FE 스택(React 18 + CRA + React Router + Zustand + Axios + Capacitor)에서의 구현 가능성
...
분석 항목:
1. 각 대안의 구현 방법 (React/CRA/JSX 기준, page/hook/API client 레이어 수준)
