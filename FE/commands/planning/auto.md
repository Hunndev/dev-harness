...
> **Tier 선택**
> - `/hb-fe:planning:auto` — **이 문서.** 일상 기획 기본값. 인터뷰/외부 조사/3관점 대안 분석 없음.
> - `/hb-fe:planning:deep` — 인터뷰(P2) + 외부 조사(P3) + 3관점 Agent Team 대안 분석(P4)까지 포함하는 full ceremony. 아키텍처급 결정에 사용.
>
> 어느 쪽이든 최종 산출물은 `decision-draft.md` → 사용자 승인 → `/hb-fe:shared:update-docs adr`로 `.harness/docs/adr.yaml`에 편입된다.
...
2. 다음을 사용자와 함께 확정한다:
   ...
   - 이해관계자 맵 (간략: 영향받는 사용자 유형 + React/CRA/React Router/Zustand 컴포넌트)
   - **모호한 논의점** 명시
...
2. 불확실성이 크고 3관점 분석이 필요하면 여기서 중단하고 `/hb-fe:planning:deep`으로 전환을 제안한다.
...
다음 중 하나라도 해당하면 P2에서 중단하고 `/hb-fe:planning:deep`으로 재시작한다:
- 대안이 3개 이상이고 우열이 명확하지 않다.
- 기존 ADR과 충돌 가능성이 있다.
- 상태·라우팅·브라우저 이벤트 흐름 재설계, API 계약 재설계 등 **한번 결정하면 되돌리기 어려운** 사안이다.
