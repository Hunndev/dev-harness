...
- CRA build와 ESLint를 항상 통과시킨다.
- page/component/hook/API/state/style 경계를 유지한다.
- 디자인 적용은 source, intent, visual, responsive, accessibility 산출물을 함께 남긴다.
...
2. sub-agent에게 전달:
   ...
   - 기존 코드 (관련 pages/hooks/API clients)
3. sub-agent는 다음을 조사:
   - 기존 코드에 비슷한 패턴이 이미 있는가? (재사용 가능성)
   - 기존 hook/API client를 확장할 수 있는가?
   - 기존 코드와 충돌할 가능성이 있는 부분은? (route 충돌, state 이름 충돌, API 계약 충돌, style cascade 충돌)
   - 기존 axios/API helper, auth token 처리, error mapping을 어떻게 활용할 것인가?
4. `prior-art.md`를 저장한다.
5. **사용자 확인 없이 자동 진행.**

### [F4] 디자인 소스 + 설계의도 작성 (Fork)

1. **worktree(fork)를 생성**하여 설계의도 문서를 작성한다.
2. 디자인 입력이 있으면 `design-source.md`를 작성한다:
   - 기준 자료: Figma URL, 스크린샷 경로, 기존 화면, 브랜드 가이드, 텍스트 설명
   - 기준 viewport와 상태: desktop/mobile, loading/empty/error/success, hover/focus
   - 불명확한 지점과 사용자 확인이 필요한 항목
3. `design-intent.md`에 아래 내용을 포함:
   - 작업 개요
   - 화면/컴포넌트 계층
   - 핵심 설계 결정과 트레이드오프 (어떤 상태를 어디에 둘지, API 호출 위치, 스타일 책임)
   - 레이아웃, 간격, 타이포, 색상, 상태 표현 원칙
   - 반응형 동작과 모바일 shell 영향
   - 의도적으로 제외한 것
   - 주의사항 (race condition, 중복 API 호출, unmounted setState, overflow 등)
4. 초안과 **모호한 논의점**을 사용자에게 제시한다.
5. 사용자 피드백을 반영하여 `design-intent.md`를 확정한다.
6. worktree를 정리한다.
...
2. **sub-agent를 호출**하여 `.harness/docs/adr.yaml`에서 관련 항목을 추출한다:
   ...
   - 관련 없는 항목은 제외
3. `.harness/docs/code-convention.yaml`에서 관련 규칙을 필터링한다 (COMP-, HOOK-, API-, STYLE-, A11Y-, TEST- 등).
...
1. **worktree(fork)를 생성**한다.
2. `requirements.md`의 MUST 수용기준을 실행 가능한 React Testing Library + Jest 테스트로 변환한다.
   - 테스트 파일: `src/__tests__/{module}.feature.{branch-name}.test.jsx` (factory 또는 test fixture 사용)
   - 하나의 수용기준(AC) = 하나의 테스트 케이스
3. `npm test -- --watchAll=false {module}.feature.{branch-name}`으로 실행하여 FAIL을 확인한다.
...
5. 실패 이유가 "올바른 이유"인지 검증한다. 자세한 검증 규칙과 재작성 루프(최대 3회)는 `commands/shared/tdd.md` 참조:
   ...
   - JS/TS 컴파일 / import / mock 오류 → Red 아님. 테스트를 수정 후 재실행 (최대 3회). 3회 후에도 올바르지 않으면 `tdd-red-debug.md`에 기록하고 사용자에게 보고.
6. worktree를 정리한다.
...
3. 기존 테스트 회귀를 확인한다: `npm test -- --watchAll=false` (전체 스위트)
4. 추가 검증: `npm run lint` + `npm run build`
...
3. 중복 제거, 네이밍 개선, 구조 정리를 수행한다.
   - **새 기능 금지, 테스트 변경 금지**
4. 각 리팩토링 후 `npm test -- --watchAll=false`로 전체 그린을 확인한다. 깨지면 즉시 revert.
...
### [F9] 시각/반응형/a11y + API 바인딩·E2E 검증 (Fork)
...
7. **E2E 렌즈가 걸린 작업이면** `e2e-check.md`에 기록한다: 실행 환경(`local-mock`/`local-dev-api`/`actual-dev`), user-inst 독립 context(시나리오에 필요한 **역할별 독립 context** — 양계정 참여 시 `user`/`inst` 각각, 단일 역할은 해당 계정만; cookie·localStorage·sessionStorage·auth 격리), 시나리오별 `정상/비정상/미확인` 판정과 screenshot/video/trace 경로, 기존 spec 재사용/신규 구분, `actual-dev` 생성 데이터 범위(고유 실행 ID)·cleanup 여부. 렌즈·안전경계 정의는 `CLAUDE.md`의 "E2E 검증 렌즈 (Playwright)" 절 참조. (해당 없으면 "N/A: E2E 미해당")
8. 디자인 소스와 다르게 구현한 부분은 의도와 근거를 남긴다.
9. 깨짐·상태 누락이 있거나 **E2E spec 갭·setup 부재(Playwright dependency/config/script)가 발견되면** F7로 복귀하여 수정한다 — spec 갭·setup 부재는 **구현 gap**으로 구현 단계(F7)가 신규 spec·setup 생성·검증을 담당하고(TDD Red 범위 제한의 예외), 완료 후 F9 E2E를 다시 수행한다.
10. worktree를 정리한다.
...
3. PR 본문 구조:
   - Summary (1-3문장)
   - Changes (모듈 단위: pages/components/hooks/API clients/routes/styles 등)
   - Breaking Changes
   - Test Plan (React Testing Library + Jest 테스트 + visual/responsive/a11y 확인)
   - API/Env Changes (BE 연동 또는 환경변수 변경)
...
2. 입력:
   ...
   - `design-intent.md`
   - `visual-check.md`, `responsive-check.md`, `accessibility-notes.md`, `api-binding-check.md`, `e2e-check.md` (E2E 렌즈가 걸린 작업일 때)
   - `pr-body.md`
...
4. QA 수행:
   ...
   - 시각/반응형/a11y 산출물이 최신인지 재확인하고, API 바인딩 작업이면 `api-binding-check.md`도, E2E 렌즈가 걸린 작업이면 `e2e-check.md`(미확인 잔존 여부 포함)도 최신인지 재확인한다.
5. 버그 발견 시 수정 루프.
