...
> **Tier 선택**
> - `/hb-fe:feature:auto` — **이 문서.** 일반 기능/화면 기본값. deep의 prior-art, 평가기준 재생성, PR본문 Fork를 경량화한다.
> - `/hb-fe:feature:deep` — full ceremony. 공통 컴포넌트, 디자인 시스템, 라우팅/상태관리, 모바일 shell 영향이 클 때 사용.

## 사전 조건

- `.harness/docs/code-convention.yaml`과 `.harness/docs/adr.yaml`이 작성되어 있어야 한다.
- 요구사항이 확정되어 있어야 한다. 확정되지 않았으면 `/hb-fe:planning:auto` 또는 `:deep`으로.
...
### [F3] 디자인 소스 + 설계의도 작성 (메인) _(deep의 F4에 해당)_
...
3. 아래 내용을 포함하는 `design-intent.md`를 작성:
   - 작업 개요
   - 화면/컴포넌트 계층
   - 핵심 설계 결정과 트레이드오프
...
### [F7] 시각/반응형/a11y + API 바인딩·E2E 검증 (Fork) _(deep의 F9에 해당)_

1. worktree(fork)를 생성한다.
2. 로컬 앱을 실행하거나 기존 dev 서버를 사용해 주요 route를 확인한다.
3. `visual-check.md`에 다음을 기록한다:
   - 확인한 route와 사용자 상태
   - desktop/mobile 관찰 결과 또는 캡처 위치
...
7. **E2E 렌즈가 걸린 작업이면** `e2e-check.md`에 기록한다: 실행 환경(`local-mock`/`local-dev-api`/`actual-dev`), user-inst 독립 context(시나리오에 필요한 **역할별 독립 context** — 양계정 참여 시 `user`/`inst` 각각, 단일 역할은 해당 계정만; cookie·localStorage·sessionStorage·auth 격리), 시나리오별 `정상/비정상/미확인` 판정과 screenshot/video/trace 경로, 기존 spec 재사용/신규 구분, `actual-dev` 생성 데이터 범위(고유 실행 ID)·cleanup 여부. 렌즈·안전경계 정의는 `CLAUDE.md`의 "E2E 검증 렌즈 (Playwright)" 절 참조. (해당 없으면 "N/A: E2E 미해당"으로 표기)
8. 깨짐·상태 누락이 있거나 **E2E spec 갭·setup 부재(Playwright dependency/config/script)가 발견되면** F5로 복귀하여 수정한다 — spec 갭·setup 부재는 **구현 gap**으로 구현 단계(F5)가 신규 spec·setup 생성·검증을 담당하고(TDD Red 범위 제한의 예외), 완료 후 F7 E2E를 다시 수행한다.
9. worktree를 정리한다.
...
2. 입력:
   - `design-intent.md` (의도)
   - `visual-check.md`, `responsive-check.md`, `accessibility-notes.md`, `api-binding-check.md` (FE 검증: 디자인 + 바인딩), `e2e-check.md` (E2E 렌즈가 걸린 작업일 때)
