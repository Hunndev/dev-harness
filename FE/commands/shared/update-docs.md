...
```
/hb-fe:shared:update-docs                — 전체 문서 대상으로 변경 필요 항목 분석
/hb-fe:shared:update-docs convention     — code-convention.yaml만
/hb-fe:shared:update-docs adr            — adr.yaml만
/hb-fe:shared:update-docs architecture   — architecture.yaml만
/hb-fe:shared:update-docs modules        — module-registry.yaml만
```
...
```yaml
- id: {카테고리}-{번호}    # GEN, FE, REACT, A11Y, TEST, GIT
  rule: ...                 # 명확하고 실행 가능한 규칙
  stacks: [...]             # react, cra, router, zustand, a11y, all 등
```
...
- 나쁜 예: "API 호출이 불안정해서 설정을 변경"
- 좋은 예: "모바일 viewport에서 상품 카드의 긴 제목이 가격 영역을 밀어내 CTA가 접히는 문제가 반복되어, 카드 타이포/줄수/overflow 기준을 명문화한다."
