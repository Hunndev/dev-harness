...
- **재사용 가드**: F7/F9 Fork(또는 직전 실행)가 이미 실행해 남긴 `e2e-check.md`가 있으면, 거기 기록된 **검사 시점 HEAD SHA·E2E 대상 source/spec 파일 content fingerprint·환경·시나리오**가 현재와 **모두 동일할 때만 재실행하지 않고 그 결과를 재사용**한다 — 하나라도 다르면 재실행한다.
