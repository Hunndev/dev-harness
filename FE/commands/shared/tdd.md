...
프레임워크: **React 18 + CRA + React Testing Library + Jest**

---

## Pre-flight 점검 (모든 TDD 트랙의 F1/M1/H1 시작 시 필수)

각 트랙이 TDD 사이클에 진입하기 전에 **반드시** 다음 2가지를 확인한다. 실패 시 사이클을 시작하지 않고 사용자에게 명확한 원인과 함께 중단을 보고한다. 이 체크는 "Red가 구현 부재 때문인가, 환경 설정 때문인가"를 사전에 구별해 false-escalation을 방지한다. 단, 기존 테스트가 하나도 없는 초기 FE 레포도 통과할 수 있어야 한다.

### 1. 테스트 러너 사전 검증 (fail-fast)

```bash
npm test -- --watchAll=false --passWithNoTests --listTests
node -p "require('react-scripts/package.json').version"
```

- exit 0 → CRA test runner와 React Testing Library + Jest 사용 가능 → 사이클 진행
- exit ≠ 0 → 즉시 중단하고 사용자에게 보고:
  - "`react-scripts test` 실행 불가" OR
  - "`src/setupTests.js` 또는 `package.json`의 jest 설정 문제" OR
  - 실제 에러 메시지 그대로 전달
- target test 실행은 기본적으로 positional 패턴(`npm test -- --watchAll=false {module}`)을 사용한다. Jest 버전별 `--testPathPattern`/`--testPathPatterns` 차이를 피하기 위해서다. 꼭 flag가 필요하면 target repo의 Jest 버전을 확인한 뒤 맞는 이름을 선택한다.
...
3. 아래 Test Design 항목을 검수하고 `tdd-design-metadata.json` 입력을 준비한 뒤 부모 CLI로 baseline을 직접 실행한다. `--test-file`은 실제 테스트 파일의 저장소 상대 경로다. `--issue-type`에는 실제 작업 유형(`feature`, `bug`, `performance`, `refactor`, `hotfix`)을 넣는다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check red --repo <root> --test-file {test-file} --cmd 'npm test -- --watchAll=false {module}' --design {artifacts-dir}/tdd-design-metadata.json --out {artifacts-dir} --issue-type {issue-type}
   ```
...
4. **실패 이유 검증 (bug/feature 유형)**:
   ...
   - JS/TS 컴파일 에러 / import 에러 / mock 설정 오류 → **올바르지 않은 Red**
...
4. 관련 회귀·위험도 검수를 마치고 `tdd-sensitivity-metadata.json`을 준비한 뒤 부모 CLI로 변경 후 테스트를 실행한다. `--design`에는 Red에서 생성된 **동일한 고정 Design 파일**을 전달한다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check green --repo <root> --test-file {test-file} --cmd 'npm test -- --watchAll=false {module}' --design {artifacts-dir}/tdd-test-design-result.json --sensitivity {artifacts-dir}/tdd-sensitivity-metadata.json --out {artifacts-dir} --issue-type {issue-type}
   ```
...
4. 각 리팩토링 후 전체 테스트 재실행. 깨지면 즉시 revert.
   ...
   ```bash
   npm test -- --watchAll=false {module}
   ```
...
꼭 flag가 필요하면 target repo의 Jest 버전을 확인 후 맞는 이름을 선택한다.

---

## React/CRA/Browser UI Test Harness 카탈로그
...
| 영역 | 해결책 |
|------|------|
| 컴포넌트 렌더링 | `render()` + semantic query(`getByRole`, `getByLabelText`) 우선 |
| 사용자 상호작용 | `@testing-library/user-event`, 필요 시 `fireEvent` |
| 라우팅 | `MemoryRouter`로 route와 navigation state 구성 |
| API client | MSW, axios mock, 또는 jest mock 중 target repo 패턴 우선 |
| 인증 상태 | Zustand/user store seed, localStorage/sessionStorage mock |
| 브라우저 API | `matchMedia`, `ResizeObserver`, `IntersectionObserver`, clipboard mock |
| 폼 검증 | `react-hook-form` + zod schema와 실제 에러 메시지 검증 |
| async UI | loading/empty/error/success 상태를 `findBy*`, `waitFor`로 검증 |
| 스타일/반응형 | jsdom 한계가 있으므로 브라우저 visual check 산출물로 보완 |
| 성능 | render count, 중복 API call count, 큰 이미지/번들 영향 기록 |
