...
- 에러 로그, 재현 절차, 스크린샷, 브라우저 이벤트 로그 등이 있으면 제공한다.

## 식별자

이슈 ID가 있으면 그대로 사용. 없으면 `maint-YYYYMMDD-slug` 형식.
예: `BUCCL-FE-42` 또는 `maint-20260406-route-state-reset`
...
2. 사용자가 제시한 이슈를 정리한다.
3. 이슈 유형을 분류한다:
   - `bug` — 화면 런타임 예외, API 실패, 잘못된 표시, 브라우저 이벤트 오류
...
2. 재현 테스트 케이스를 작성한다:
   ...
   - React Testing Library + Jest mock 활용 (`src/__tests__/mocks/index.ts`로 외부 의존성 mock)
   - API는 MSW 또는 jest mock
...
3. Baseline 로그를 `.harness/artifacts/maintenance/{identifier}/tdd-baseline-log.txt`에 저장한다:
   ...
   - performance 유형: 기준선(render time, interaction latency, API 호출 횟수, 메모리)을 기록.
...
2. sub-agent에게 전달:
   - `reproduction.md`
   - 관련 모듈 코드 (pages/, hooks/, API clients/, guards/, browser-event/)
   - `.harness/docs/adr.yaml`
   - `.harness/docs/architecture.yaml`
3. sub-agent는 tracer 스타일로 분석:
   - stack trace에서 발생 지점 특정
   - request flow를 따라 문제 지점 추적 (route → guard → page → component → hook → API client → backend API)
   - 브라우저 이벤트 흐름이면: browser event → handler → state update → render
...
#### Agent A: 모듈/레이어 방향
```
다음 근본 원인을 기준으로, 수직 방향(API client → hook → component → page → route)으로
영향받는 코드를 추적하라.

분석:
1. 이 원인이 다른 API client의 API 호출/관계에 영향을 미치는가?
2. 이 원인과 같은 hook를 호출하는 다른 component가 있는가?
3. 이 원인이 브라우저 이벤트 handler 또는 listener에도 전파되는가?
...
2. React 라우터에서 직접 호출하는 경로가 있는가?
3. 브라우저 이벤트 component 또는 handler에서 호출하는 경우가 있는가?
4. route guard, hook, component tree 중 어디에 끼어 있는가?
...
분석:
1. 이 원인이 API 응답 데이터의 일관성에 영향을 미치는가?
2. 이미 잘못된 데이터가 쌓여 있을 가능성은?
3. Zustand/localStorage/sessionStorage에 stale 데이터가 있을 가능성은?
4. 메인 BE(hb-be) 또는 외부 시스템에 잘못된 데이터가 전달되었을 가능성은?
5. API 계약 변경으로 client cache/state 보정이 필요한가?
...
3. sub-agent는 다음을 확인:
   ...
   - 수정 방향이 convention 규칙(COMP-, HOOK-, API-, STYLE-, A11Y-, TEST- 등)을 위반하는가?
...
2. 수정 원칙:
   ...
   - **타입 안정성**: 불필요한 any/ts-ignore 금지, CRA build 통과
   - **API 계약 변경 분리**: 필요 시 별도 커밋
...
npm test -- --watchAll=false src/__tests__/{module}
