...
- `team_name`: `maint-impact-{identifier}` (예: `maint-impact-BUCCL-FE-42`)
...
API client → hook → component → page → route 순으로:
1. 이 원인이 다른 API client의 API 호출/관계에 영향을 미치는가?
2. 같은 hook를 호출하는 다른 component가 있는가?
3. 다른 모듈(module-registry의 다른 모듈)에 전파되는가?
4. 브라우저 이벤트 handler 또는 listener에도 영향이 있는가?
...
2. React 라우터에서 직접 호출하는 경로
3. 브라우저 이벤트 handler 또는 listener에서 호출하는 경우
...
3. Zustand/localStorage/sessionStorage에 stale 데이터가 있을 가능성
4. 메인 BE(hb-be) 또는 외부 시스템에 잘못된 데이터 전달 가능성
5. client cache/state 보정 API 계약 변경 필요 여부
...
- 영향받는 모듈: {N}개
- 영향받는 파일: {N}개
- client cache/state 보정 필요: {있음|없음}
- client cache/store 초기화 필요: {있음|없음}
- 메인 BE 연동 영향: {있음|없음}
