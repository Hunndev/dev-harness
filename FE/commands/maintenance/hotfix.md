...
> **Tier 선택**
> - `/hb-fe:maintenance:hotfix` — **이 문서.** 최소 ceremony. 영향도 분석, RCA, convention 충돌 체크 등은 전부 생략.
> - `/hb-fe:maintenance:auto` — 일상 유지보수. RCA 포함.
> - `/hb-fe:maintenance:deep` — full ceremony, 3방향 영향 Team 포함.
...
예: `BUCCL-FE-99` 또는 `hotfix-20260408-button-nullref`
...
3. 사용자가 제시한 증상을 **FAIL로 입증하는 최소 테스트**를 작성한다.
   - 파일: `src/__tests__/{module}.hotfix.{identifier}.test.jsx`
   - 가장 좁은 범위(단일 함수/component 또는 handler/API client)로 한정
4. `commands/shared/tdd.md`의 **T0 Test Design Check**로 완료기준·행동 assertion·mock 경계를 확인하고 `tdd-design-metadata.json`을 준비한다. 부모 CLI로 선택한 재현 테스트를 실행한다. `--test-file`에는 3번에서 만든 파일의 저장소 상대 경로를 넣는다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check red --repo <root> --test-file {test-file} --cmd 'npm test -- --watchAll=false {module}.hotfix.{identifier}' --design {artifacts-dir}/tdd-design-metadata.json --out <root>/.harness/artifacts/maintenance/{identifier} --issue-type hotfix
   ```
...
1. **Red 테스트(H1)가 PASS가 되는 '최소 수정'만** 수행한다. 사용자가 지정한 파일·라인 이외는 수정하지 않는다.
   ...
   - 금지 범주에 해당하면 즉시 중단하고 `/hb-fe:maintenance:auto`로 에스컬레이션한다.
...
1. `npm test -- --watchAll=false {module}`
2. 필요 시 `npm run lint`를 실행한다. lint 수정이 단순 포맷을 넘어가면 `:auto`로 전환한다.
3. 결과 확인:
   - H1 재현 테스트: **PASS**여야 함
...
> - **Compound fix 예외**: 하나의 수정이 **여러 독립적 변경의 합집합**으로 이루어질 때 (예: 두 component 또는 handler에 동시에 null 체크를 넣어야 FAIL이 해소되는 경우), **전체 합집합을 단일 fix로 간주**한다. 개별 변경을 독립 평가하지 않는다. 단, 합집합이 2개 이상 모듈에 걸치면 hotfix 범위 밖이므로 `:auto`로 에스컬레이션한다.
> 코드 정리가 필요하면 `/hb-fe:maintenance:auto`로 전환한다.
...
| 상황 | 전환 대상 |
|---|---|
| 수정 범위가 2개 이상 모듈에 걸친다 | `/hb-fe:maintenance:auto` |
| 기존 ADR/convention 위반이 의심된다 | `/hb-fe:maintenance:deep` |
| 회귀가 단위 테스트 범위를 벗어난다 | `/hb-fe:maintenance:auto` (전체 회귀 필요) |
| 새 설계 결정이 필요하다 | `/hb-fe:planning:auto` 또는 `:deep` |
| H2 수정이 2회 연속 실패 | `/hb-fe:maintenance:auto` (RCA 필요) |
| 상태·라우팅·브라우저 이벤트 흐름 변경이 필요하다 | `/hb-fe:maintenance:deep` (이벤트 흐름은 hotfix 범위 아님) |
| API 호출 스키마 변경이 필요하다 | `/hb-fe:maintenance:auto` (API 계약 변경은 hotfix 범위 아님) |
| 리팩토링이 필요하다 | `/hb-fe:maintenance:auto` (hotfix에서는 Refactor 금지) |
