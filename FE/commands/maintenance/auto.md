...
> **Tier 선택**
> - `/hb-fe:maintenance:hotfix` — 오타, 한 줄 fix, 긴급 수정. 재현 테스트 + 수정 + 단위 테스트 3단계.
> - `/hb-fe:maintenance:auto` — **이 문서.** 일상 유지보수 기본값. M4 영향도 3방향 Team, M5 ADR 충돌 체크 없음.
> - `/hb-fe:maintenance:deep` — M1~M9 full ceremony. 모듈 경계가 불분명하거나 기존 ADR 위반이 의심될 때.
...
예: `BUCCL-FE-42` 또는 `maint-20260408-mobile-overflow`

## 핵심 원칙

- 회귀 방지 최우선
- 수정 범위 폭주 방지 → `fix-plan.md`에서 범위 명시
- **신규 ADR 생성 금지** — 새 결정이 필요하면 `/hb-fe:planning:auto` 또는 `:deep`으로 에스컬레이션
...
2. 사용자가 제시한 이슈를 정리한다.
3. 이슈 유형을 분류한다:
   - `bug` — 화면 런타임 예외, API 실패, 잘못된 동작, 시각적 깨짐
   - `refactor` — 코드 구조 개선, 기술 부채 해소
   - `performance` — 느린 렌더링, 중복 API 호출, 번들/이미지 비용, 메모리
   - `dependency` — 패키지 업그레이드, 보안 패치
   - `visual` — 디자인 적용 오류, responsive overflow, focus/contrast 문제
4. `.harness/docs/module-registry.yaml`을 읽고, 관련 route/page/component/hook/API/state/style을 식별한다.
5. 사용자에게 다음을 확인한다:
   - 이슈 유형
   - 관련 모듈
...
2. 재현 테스트 케이스를 작성한다:
   - 테스트 파일: `src/__tests__/{module}.maint.{identifier}.test.jsx`
...
3. Baseline 로그를 `.harness/artifacts/maintenance/{identifier}/tdd-baseline-log.txt`에 저장한다:
   ...
   - performance 유형: 기준선(render time, API 호출 횟수, bundle/image 비용, 메모리)을 기록.
   - visual 유형: 깨진 viewport, 캡처 위치, 기대 화면을 기록.
...
2. sub-agent에게 전달:
   - `reproduction.md`
   - 관련 모듈 코드 (route, page, component, hook, API client, state, style)
...
2. 계획에 포함:
   - 수정 대상 파일 목록
   - 각 수정의 내용과 이유
   - 회귀 리스크 (상태·라우팅·브라우저 이벤트 흐름, style cascade, responsive 영향 포함)
   - **범위 제한** (이번에 하지 않는 것)
   - API 계약/환경변수/디자인 토큰 변경 필요 여부
...
2. 수정 원칙:
   ...
   - **convention 준수**: API client 경계, component/hook/style 책임, 명백한 규칙 위반 즉시 체크
   - **API 계약/환경변수 변경 분리**: 필요 시 별도 커밋
...
2. 모든 테스트가 PASS인지 먼저 확인 (`npm test -- --watchAll=false`).
...
- 상태·라우팅·브라우저 이벤트 흐름 문제가 여러 화면과 viewport에 전파된 가능성
