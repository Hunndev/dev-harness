...
- **local-mock 강화**: unhandled mock 요청은 **fail로 처리**한다. CRA/dev proxy의 upstream은 **network 발생 이전에 resolve해 allowlist guard**를 적용하며, 증명 불가하면 local-mock 실행을 **거부하고 FAIL로 기록**한다 — 충족 조치는 **구현 단계 복귀** 후 재실행한다.
- **Service worker 차단**: service worker는 browser 라우팅을 우회하므로 Playwright context에서 **SW 비활성화를 기본 강제**(예: service worker 차단 옵션)하거나 **더 낮은 계층의 proxy allowlist를 증명**해야 한다 — 둘 다 아니면 실행을 **거부하고 FAIL로 기록**한다 — 충족 조치는 **구현 단계 복귀** 후 재실행한다.
...
- **trace redaction**: 보존하는 trace는 token·cookie·auth header·response body·DOM 등 **secret·민감데이터의 redaction 또는 secret scan을 통과**해야 하며, **restricted evidence storage — 접근 제한이 확인된 `e2e-evidence/` 보존 경로(또는 사용자 지정 제한 저장소) — 에만 보존**한다. 안전을 증명하지 못하면 trace 보존을 **금지**하고, 다른 허용 증거(**video** — 없으면 video 기록을 켜고 **재실행**)로 `비정상`/`미확인` 증거 계약을 충족한다.
