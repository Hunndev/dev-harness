...
`git branch --show-current`로 resolve한다. `{track}`은 호출 맥락(feature/maintenance/planning)을 따른다.
...
2. `.harness/artifacts/{track}/{identifier}/`에서 기준 문서를 연결한다:
...
2. 없으면 → 통과 ✅. `evaluate-report.md`를 확정한다.
...
- branch: {branch-name}
- 변경 파일 수: {n}
- 기준 출처: {code-quality-guide.md 또는 .harness/docs/code-convention.yaml + .harness/docs/adr.yaml}
