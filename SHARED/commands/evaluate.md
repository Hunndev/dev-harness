...
- **완료기준·증거·리뷰 렌즈는 각 플러그인(스택)을 따른다.** 여기서 명령으로 하드코딩하지 않는다.
  ...
  - 예: FE = [디자인] 시각·UX·반응형·접근성 + Claude 디자인 검증 / [API 바인딩] 계약 일치·상태 처리(loading/empty/error/success)·mock 잔재 없음·api 계층(src/api·src/utils/api.js) 경유 — 모드 분류는 FE 플러그인.
  - 예: CHAT = 테스트·lint·build·tsc + 계약 검증(websocket-events·api-contract 등록 여부) + dual review gate.
  - 예: AOS/IOS = 테스트·lint·빌드(gradlew / xcodebuild) + 모바일 검증([shell] 기기·권한·푸시·딥링크 / [브리지] 계약 일치·형제 플랫폼 동일성).
...
feature 트랙은 `git branch --show-current`, maintenance 트랙은 트랙 파이프라인의 issue-id, planning은 `plan-YYYYMMDD-slug`로 resolve한다 — **seed가 쓴 식별자와 동일**해야 한다. `{track}`은 호출 맥락(feature/maintenance/planning)을 따른다.
...
3. **완료기준을 어디서 가져올지 스택에게 위임**한다: 호출한 플러그인(BE/CM/FE/CHAT/AOS/IOS)의 검증 규칙을 따른다. 명령은 "무엇을 검사할지"를 박지 않고 "스택 기준으로 검사하라"만 지시한다.
...
1. **Sub-agent를 호출**하여 스택이 정의한 자동검사를 실행한다 (해당 플러그인의 `shared/verify` 명령):
   ...
   - FE: 빌드·린트 + 시각·반응형·접근성 검사 + Claude 디자인 검증 (API 바인딩 작업은 계약·상태·mock 검사).
   - CHAT: 테스트·lint·build·tsc + 계약 검증(websocket-events·api-contract·database-schema 반영 여부).
   - AOS/IOS: 단위 테스트 + lint·빌드(gradlew testDebugUnitTest·lint·assembleDebug / xcodebuild test·build) + 모바일 검증(기기·권한·푸시·딥링크 — 브리지 계약 작업은 bridge-check).
...
- 적용 스택: {BE/CM/FE/CHAT/AOS/IOS} (완료기준은 이 스택을 따름)
