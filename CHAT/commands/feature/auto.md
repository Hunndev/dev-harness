...
> **Tier 선택**
> - `/hb-chat:feature:auto` — **이 문서.** 일반 기능 기본값. F3 유사 구현 조사, F5 평가기준 재생성, F9 PR본문 Fork를 생략한다.
> - `/hb-chat:feature:deep` — F1~F11 full ceremony. 복잡 기능, 이벤트 흐름 상호작용이 크거나 PR 문서화가 중요할 때 사용.

## 사전 조건

- `.harness/docs/code-convention.yaml`과 `.harness/docs/adr.yaml`이 작성되어 있어야 한다.
- 이번 기능이 건드리는 chat 계약 문서(`websocket-events.yaml`, `api-contract.yaml`, `database-schema.yaml`)가 준비돼 있어야 한다. 없으면 `/hb-chat:contract:websocket` 또는 `:api`로 먼저 정의한다.
- 요구사항이 확정되어 있어야 한다. 확정되지 않았으면 `/hb-chat:planning:auto` 또는 `:deep`으로.
...
4. `contract-check.md`(pre 섹션)를 저장한다.
5. worktree를 정리한다.
...
2. 입력:
   - `design-intent.md` (의도)
   - `contract-check.md` (F3b 계약 점검 — 계약 준수 여부 확인)
...
2. 각 코멘트의 수용/거부 판단을 사용자에게 제시:
   ...
   - [p4]: 일괄 처리.
3. 사용자 확인 후 코드를 수정한다.
4. QA 수행 (`commands/shared/verify.md` 1~5단계):
   - `npm test` / `npm run lint` / `npx tsc --noEmit` / `npm run build`
   - `tdd-green-log.txt`가 여전히 PASS 상태인지 재확인한다.
   - **계약 점검(post)**: `commands/feature/contract-check.md`의 post 절차로 코드↔계약 일치를 검증한다(불일치=[p1]).
5. **Dual review gate** (`commands/shared/review-gates.md`): F7 Claude 리뷰 + Codex 리뷰 모두 blocking(=[p1]) 0이어야 통과. 미충족 시 수정 루프(최대 3라운드).
