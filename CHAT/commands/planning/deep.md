...
2. 다음을 사용자와 함께 확정한다:
   ...
   - 이해관계자 맵 (누가 이 결정에 영향을 받는가)
     - 사용자 유형별 (채팅 사용자, 강사, 운영자)
     - 시스템 유형별 (chat 서비스 자체, 메인 BE, MySQL/Redis, Socket.io 클라이언트)
   - **모호한 논의점** 제시
...
3. sub-agent는 다음을 조사한다:
   - 유사 서비스/기능의 구현 사례 (실시간 메시징/presence/첨부 플랫폼)
   - 관련 npm 패키지, Express 미들웨어, Socket.io 패턴
   - 알려진 함정이나 안티패턴 (N+1 쿼리, EventLoop blocking, memory leak, race condition)
   - BUCCL Chat 스택(Node 18 + TS + Express + MySQL + Redis + Socket.io)에서의 구현 가능성
...
분석 항목:
1. 각 대안이 채팅 사용자/강사/운영자에게 미치는 영향
