...
> **Tier 선택**
> - `/hb-chat:maintenance:hotfix` — **이 문서.** 최소 ceremony. 영향도 분석, RCA, convention 충돌 체크 등은 전부 생략.
> - `/hb-chat:maintenance:auto` — 일상 유지보수. RCA 포함.
> - `/hb-chat:maintenance:deep` — full ceremony, 3방향 영향 Team 포함.
...
2. `npm test -- --testPathPattern={module}`
   > `npm run lint`는 hotfix에서는 실행하지 않는다 — 스타일 위반이 있으면 `:auto`로 전환한다.
3. 결과 확인:
   - H1 재현 테스트: **PASS**여야 함
...
| 상황 | 전환 대상 |
|---|---|
| 수정 범위가 2개 이상 모듈에 걸친다 | `/hb-chat:maintenance:auto` |
| 기존 ADR/convention 위반이 의심된다 | `/hb-chat:maintenance:deep` |
| 회귀가 단위 테스트 범위를 벗어난다 | `/hb-chat:maintenance:auto` (전체 회귀 필요) |
| 새 설계 결정이 필요하다 | `/hb-chat:planning:auto` 또는 `:deep` |
| H2 수정이 2회 연속 실패 | `/hb-chat:maintenance:auto` (RCA 필요) |
| Socket.io 이벤트 흐름 변경이 필요하다 | `/hb-chat:maintenance:deep` (이벤트 흐름은 hotfix 범위 아님) |
| SQL 스키마 변경이 필요하다 | `/hb-chat:maintenance:auto` (마이그레이션은 hotfix 범위 아님) |
| 리팩토링이 필요하다 | `/hb-chat:maintenance:auto` (hotfix에서는 Refactor 금지) |
