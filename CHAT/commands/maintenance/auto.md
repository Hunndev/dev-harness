...
> **Tier 선택**
> - `/hb-chat:maintenance:hotfix` — 오타, 한 줄 fix, 긴급 수정. 재현 테스트 + 수정 + 단위 테스트 3단계.
> - `/hb-chat:maintenance:auto` — **이 문서.** 일상 유지보수 기본값. M4 영향도 3방향 Team, M5 ADR 충돌 체크 없음.
> - `/hb-chat:maintenance:deep` — M1~M9 full ceremony. 모듈 경계가 불분명하거나 기존 ADR 위반이 의심될 때.
...
- **신규 ADR 생성 금지** — 새 결정이 필요하면 `/hb-chat:planning:auto` 또는 `:deep`으로 에스컬레이션
...
> fix-plan 범위를 벗어나는 리팩토링 유혹이 생기면 `/hb-chat:maintenance:deep`으로 에스컬레이션하거나 별도 maintenance 작업으로 분리한다.
...
> 완료는 `commands/shared/review-gates.md`(Codex + Claude dual gate)를 통과해야 한다. 변경이 Socket 이벤트·REST·DB 계약을 건드리면 `commands/feature/contract-check.md`로 정합성을 확인한다.
