...
> chat repository가 `.harness/docs/websocket-events.yaml`을 Socket.io 계약의 진실의 원천으로 선언한다. 실제 event handler·test와 충돌하면 계약을 자동으로 무시하지 않고 `BLOCKED` 후 코드 위반인지 승인된 계약 변경의 문서 drift인지 판정한다.
