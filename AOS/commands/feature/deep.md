...
- `./gradlew assembleDebug`와 `./gradlew lint`를 항상 통과시킨다.
- webview/bridge/network/fcm/utils 경계를 유지한다.
- shell·브리지 작업은 설계의도(design-intent)와 두 모드 산출물(device/permission/release/bridge-check)을 함께 남긴다.
...
1. **Pre-flight 점검**: `commands/shared/tdd.md`의 "Pre-flight 점검" 섹션을 수행한다:
   - `./gradlew testDebugUnitTest --dry-run` → exit 0 확인 (아니면 중단 + 사용자 보고)
   - `./gradlew --version`으로 Gradle/JDK 버전 확인(정보용). target test는 `--tests` 와일드카드 패턴 사용
...
2. 작업 모드를 분류한다: **shell 기능 / 브리지 계약 / 혼합** (CLAUDE.md "AOS 작업 두 모드" 참조).
3. planning 트랙의 산출물이 있으면 가져온다:
   - `requirements-interview.md` → 요약
   - `decision-draft.md` 또는 관련 `.harness/docs/adr.yaml` 항목 → 참조
4. 없으면 사용자에게 질문하여 수집한다.
5. 요구사항을 MUST / SHOULD / NICE로 분류한다.
6. `requirements.md`를 저장한다.
7. worktree를 정리한다.
...
2. sub-agent에게 전달:
   ...
   - 기존 코드 (관련 bridge/webview/network/fcm 소스)
3. sub-agent는 다음을 조사:
   - 기존 코드에 비슷한 패턴이 이미 있는가? (재사용 가능성)
   - 기존 브리지 함수/WebView 설정/헬퍼를 확장할 수 있는가?
   - 기존 코드와 충돌할 가능성이 있는 부분은? (브리지 함수명 충돌, intent-filter 충돌, 권한 중복, 쿠키 도메인 충돌)
   - 기존 `WebAppInterface`, `WebViewCookieJar`/`AuthCookieSyncDecisions`, FCM 헬퍼를 어떻게 활용할 것인가?
...
2. `design-intent.md`에 아래 내용을 포함:
   - 작업 개요와 작업 모드 (shell 기능 / 브리지 계약 / 혼합)
   - 관련 컴포넌트 계층 (Activity / webview / bridge / network / fcm / manifest)
   - 핵심 설계 결정과 트레이드오프 (어떤 로직을 어느 계층에 둘지, 브리지 함수 시그니처, 쿠키 동기화 책임)
   - WebView 설정·권한·푸시·딥링크 영향
   - 브리지 계약 변경 여부 — 있으면 `bridge-contract.yaml` 갱신 계획과 형제 플랫폼(iOS) 반영 필요 여부
   - 의도적으로 제외한 것
   - 주의사항 (race condition, 중복 브리지 호출, WebView 생명주기, 백그라운드 푸시 제한 등)
...
2. **sub-agent를 호출**하여 `.harness/docs/adr.yaml`에서 관련 항목을 추출한다:
   ...
   - 관련 없는 항목은 제외
3. `.harness/docs/code-convention.yaml`에서 관련 규칙을 필터링한다 (BRIDGE-, WEBVIEW-, NET-, PUSH-, PERM-, TEST- 등).
...
1. **worktree(fork)를 생성**한다.
2. `requirements.md`의 MUST 수용기준을 실행 가능한 JUnit 테스트로 변환한다.
   - 테스트 파일: `app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Feature{Slug}Test.kt` (slug는 CamelCase로 변환, fake/fixture 사용)
   - 하나의 수용기준(AC) = 하나의 테스트 케이스
3. `./gradlew testDebugUnitTest --tests "*{Module}Feature{Slug}*"`으로 실행하여 FAIL을 확인한다.
...
5. 실패 이유가 "올바른 이유"인지 검증한다. 자세한 검증 규칙과 재작성 루프(최대 3회)는 `commands/shared/tdd.md` 참조:
   ...
   - Kotlin 컴파일 / import / mock 오류 → Red 아님. 테스트를 수정 후 재실행 (최대 3회). 3회 후에도 올바르지 않으면 `tdd-red-debug.md`에 기록하고 사용자에게 보고.
6. worktree를 정리한다.
...
3. 기존 테스트 회귀를 확인한다: `./gradlew testDebugUnitTest` (전체 스위트)
4. 추가 검증: `./gradlew lint` + `./gradlew assembleDebug`
...
3. 중복 제거, 네이밍 개선, 구조 정리를 수행한다.
   - **새 기능 금지, 테스트 변경 금지**
4. 각 리팩토링 후 `./gradlew testDebugUnitTest`로 전체 그린을 확인한다. 깨지면 즉시 revert.
...
### [F9] 기기/권한/릴리즈 + 브리지 계약 검증 (Fork)

1. **worktree(fork)를 생성**한다.
2. 에뮬레이터 또는 실기기에 앱을 설치·실행하여 주요 화면이 정상 로드되는지 확인한다.
3. `device-check.md`에 확인한 기기/OS 버전, 주요 화면 로드·푸시 수신·딥링크 진입·권한 흐름 확인 결과를 기록한다.
4. `permission-check.md`에 요청 권한 목록과 근거, 요청 타이밍, 거부 시 동작을 기록한다. (권한 영향 없으면 "N/A: 권한 영향 없음")
5. `release-check.md`에 versionCode/versionName, 서명 설정, 스토어 심사 체크 항목을 기록한다. (릴리즈 영향 없으면 "N/A: 릴리즈 영향 없음")
6. **브리지 계약 작업이면** `bridge-check.md`에 기록한다: 함수 시그니처·메시지 포맷·에러 처리의 웹↔네이티브 일치, `.harness/docs/bridge-contract.yaml` 갱신 여부, **형제 플랫폼(iOS) 반영 필요 여부** — 반영이 필요하면 `parity-proposal.md`(ios-buccl용 반영 제안 초안)를 작성한다. (shell 기능 전용 작업이면 "N/A: shell 기능 전용")
7. 기대 동작과 다르게 구현한 부분은 의도와 근거를 남긴다.
8. 깨짐·흐름 누락이 있으면 F7로 복귀하여 수정한다.
9. worktree를 정리한다.
...
3. PR 본문 구조:
   - Summary (1-3문장)
   - Changes (모듈 단위: bridge/webview/network/fcm/manifest/utils 등)
   - Breaking Changes
   - Test Plan (JUnit 테스트 + 기기/권한/릴리즈 확인)
   - 브리지 계약/매니페스트 변경 (형제 플랫폼(iOS) 반영 필요 여부 포함)
...
2. 입력:
   ...
   - `design-intent.md`
   - `device-check.md`, `permission-check.md`, `release-check.md`, `bridge-check.md`
   - `pr-body.md`
   - **F6 Red 테스트 파일** (`app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Feature{Slug}Test.kt`)
...
3. 리뷰 원칙:
   ...
   - shell 기능 시: 권한 과다, 푸시 수신 경로, 딥링크 충돌, WebView 보안 설정(JS 인터페이스 노출 범위) 확인
   - 브리지 계약 시: 함수 시그니처·메시지 포맷 일치, 에러 처리 누락, 형제 플랫폼(iOS)과의 계약 동일성, WebAppInterface 단일 계층 우회 확인
...
3. 사용자 확인 후 코드를 수정한다.
4. QA 수행:
   - `./gradlew assembleDebug` (빌드 검사)
   - `./gradlew lint` (Android Lint)
   - `./gradlew testDebugUnitTest --tests "*{Module}*"` (관련 모듈 테스트)
   - `./gradlew testDebugUnitTest` (전체 회귀, 필요 시)
...
- 산출물 목록
- 생성/변경된 파일 목록
- 브리지 계약/매니페스트 변경 내역 목록 (있으면)
- 테스트 결과 요약
- **형제 플랫폼(iOS) 반영 필요 여부** (필요하면 `parity-proposal.md` 경로 명시)
- 커밋 메시지 제안
