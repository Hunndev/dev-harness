...
> **Tier 선택**
> - `/hb-aos:feature:auto` — **이 문서.** 일반 shell 기능/브리지 작업 기본값. deep의 prior-art, 평가기준 재생성, PR본문 Fork를 경량화한다.
> - `/hb-aos:feature:deep` — full ceremony. 브리지 계약, 쿠키/세션 동기화, 푸시·딥링크 구조, 매니페스트 권한 영향이 클 때 사용.

## 사전 조건

- `.harness/docs/code-convention.yaml`과 `.harness/docs/adr.yaml`이 작성되어 있어야 한다.
- 요구사항이 확정되어 있어야 한다. 확정되지 않았으면 `/hb-aos:planning:auto` 또는 `:deep`으로.
...
- **작업 모드 분류 먼저**: 이 작업이 shell 기능인지 브리지 계약인지(혼합인지) F2에서 분류하고, 그에 맞는 산출물·리뷰 렌즈를 적용한다.
- **(deep의) F3 유사 구현 조사 없음**: 명백한 재사용 대상이 없거나 소규모 기능 추가 시 사용. 기존 브리지/WebView 코드 분석이 중요하면 `:deep`.
...
1. 메인에서 직접 수행한다 — 이 스텝은 문서 산출물만 만들고 코드를 수정하지 않는다. (대규모 조사만 Sub-agent 위임)
2. 아래 내용을 포함하는 `design-intent.md`를 작성:
   ...
   - 의도적으로 제외한 것
   - 주의사항 (쿠키/세션 동기화·딥링크 진입 흐름·백그라운드 푸시 영향 포함)
...
### [F7] 기기/권한/릴리즈 + 브리지 계약 검증 (Fork) _(deep의 F9에 해당)_

1. worktree(fork)를 생성한다.
2. 에뮬레이터 또는 실기기에 앱을 설치·실행하여 주요 화면이 정상 로드되는지 확인한다.
3. `device-check.md`에 다음을 기록한다:
   - 확인한 기기/OS 버전 (에뮬레이터/실기기 구분)
   - 주요 화면 로드·푸시 수신·딥링크 진입·권한 흐름 확인 결과
   - 기대 동작과 다른 부분 및 의도적 차이
...
6. **브리지 계약 작업이면** `bridge-check.md`에 기록한다: 함수 시그니처·메시지 포맷·에러 처리의 웹↔네이티브 일치, `.harness/docs/bridge-contract.yaml` 갱신 여부, **형제 플랫폼(iOS) 반영 필요 여부** — 반영이 필요하면 `parity-proposal.md`(ios-buccl용 반영 제안 초안)를 작성한다. (shell 기능 전용 작업이면 "N/A: shell 기능 전용"으로 표기)
7. 깨짐·흐름 누락이 있으면 F5로 복귀하여 수정한다.
8. worktree를 정리한다.
...
2. 입력:
   - `design-intent.md` (의도)
   - `device-check.md`, `permission-check.md`, `release-check.md`, `bridge-check.md` (AOS 검증: shell + 브리지)
   - **F4 Red 테스트 파일** (`app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Feature{Slug}Test.kt`)
...
3. 리뷰 원칙:
   ...
   - WebView 설정·쿠키 동기화·권한 처리가 Activity에 흩뿌려지지 않고 담당 계층(webview/network/bridge)을 경유하는지 확인
   - shell 렌즈: 권한 과다, 푸시 수신 경로, 딥링크 충돌, WebView 보안 설정(JS 인터페이스 노출 범위) 확인
   - 브리지 렌즈: 함수 시그니처, 메시지 포맷, 에러 처리, 형제 플랫폼(iOS)과의 계약 동일성 확인
...
- 브리지 계약·푸시·딥링크·권한을 동시에 건드리는 대형 변경이다
