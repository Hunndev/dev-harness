...
.harness/artifacts/{track}/{identifier}/eval-review/    ← `--output-root`. 비어 있어야 하고 packet source 밖이어야 한다
  ...
  review-claude/               ← 임시: Review provider 작업 디렉터리. Review가 실행됐을 때만
  review-codex/                ← 임시: Review provider 작업 디렉터리. Review가 실행됐을 때만
