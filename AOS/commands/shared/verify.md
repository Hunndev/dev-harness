...
```
Kotlin 컴파일 오류, 리소스/매니페스트 오류, 의존성 해석 실패를 차단한다.

### 2. 린트
```bash
./gradlew lint
```
Android Lint 위반(보안·성능·정합성 규칙) 차단.

### 3. 테스트 실행
```bash
./gradlew testDebugUnitTest --tests "*{Module}*"    # 대상 모듈
./gradlew testDebugUnitTest                          # 전체 (회귀 확인)
```
...
- 50줄 초과 함수 (GEN-001)
- Activity에 WebView 설정/쿠키/권한 로직 과도하게 집중 (WEBVIEW-001)
- 브리지 단일 계층(WebAppInterface) 우회한 웹→네이티브 진입 (BRIDGE-001)
- 쿠키/세션 동기화 판단 로직 흩뿌림 (NET-001)
- 매니페스트 권한 과다·intent-filter 중복 (PERM-001)
- ProGuard/R8 keep 규칙 누락 (브리지 메서드 난독화 제거 위험) (BUILD-001)
