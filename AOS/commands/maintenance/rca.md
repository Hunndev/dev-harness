...
2. architecture.yaml의 flow를 따라 문제 지점을 역추적하라.
   - 화면: intent/딥링크 → Activity → WebView(BucclWebViewClient) → bridge(WebAppInterface) → network(쿠키 동기화) → 웹(FE)
...
AOS(Kotlin/WebView) 특화 확인 항목:
- WebView 설정 오류 (JavaScript 허용, domStorage, mixed content, User-Agent)
- @JavascriptInterface 노출 범위·시그니처 불일치 (웹 호출명 ↔ WebAppInterface.kt ↔ bridge-contract.yaml)
- 쿠키/세션 동기화 누락 (WebViewCookieJar ↔ CookieManager, AuthCookieSyncDecisions 판단 로직)
- FCM 토큰 갱신/수신 경로 (포그라운드/백그라운드 분기, 알림 채널)
- 딥링크 intent-filter 매칭 실패·중복, Activity launchMode
- 런타임 권한 요청 타이밍·거부 상태 처리 (CAMERA·POST_NOTIFICATIONS 등)
- ProGuard/R8 난독화로 인한 브리지 메서드 제거 (keep 규칙 누락)
- 메인 스레드 블로킹(ANR), WebView 메모리, 생명주기 콜백 순서

인프라/릴리즈 확인 항목:
- Play 스토어 릴리즈 트랙 / versionCode·versionName
- 서명 설정(jks) / 빌드 variant 차이 (debug vs release)
- FCM 프로젝트 설정 (google-services.json)
