...
2. 버그를 재현하는 JUnit 테스트를 작성한다:
   - 파일: `app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Maint{Identifier}Test.kt` (identifier는 CamelCase로 변환)
   - 외부 의존성은 target repo의 기존 fake/mock 패턴을 우선 사용
   - 네트워크는 OkHttp MockWebServer, 인터페이스 fake 중 적합한 것 선택 (Android 프레임워크 의존은 인터페이스로 격리)
   - 현재 상태에서 **FAIL** 확인 (`./gradlew testDebugUnitTest --tests "*{Module}Maint{Identifier}*"`)
3. FAIL 출력을 `.harness/artifacts/maintenance/{identifier}/tdd-baseline-log.txt`에 캡처한다. 실패 이유가 **'올바른 이유'(버그 때문)**인지 확인한다. Kotlin 컴파일/import/mock 오류이면 Red 재작성 (최대 3회, 자세한 규칙은 `commands/shared/tdd.md` 참조).
...
1. 성능 기준선을 측정한다:
   ...
   - Android Studio Profiler 또는 logcat 타이밍 관찰
...
- [ ] 로그인/쿠키·세션 동기화(WebViewCookieJar·AuthCookieSyncDecisions) 관련인가?
- [ ] 브리지 함수(WebAppInterface) 시그니처·메시지 포맷 관련인가?
- [ ] WebView 설정(JS 허용·domStorage·mixed content·User-Agent) 관련인가?
- [ ] FCM 푸시 수신 경로(포그라운드/백그라운드, 알림 채널) 관련인가?
- [ ] 딥링크 intent-filter 매칭·Activity launchMode 관련인가?
- [ ] 런타임 권한(CAMERA·POST_NOTIFICATIONS 등) 요청·거부 흐름 관련인가?
- [ ] ProGuard/R8 난독화(브리지 메서드 keep 규칙) 관련인가?
- [ ] 기기/OS 버전별 WebView 구현 차이 관련인가?
- [ ] 메인 BE(hb-be) 연동 / JWT 공유 관련인가?
- [ ] 웹(FE) 배포 변경(브리지 호출부·쿠키 도메인) 관련인가?
- [ ] Play 스토어 릴리즈/서명/versionCode 관련인가?
...
## 재현 테스트
- 파일: app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Maint{Identifier}Test.kt
- 테스트명: {class} > {method}
