...
> **Tier 선택**
> - `/hb-aos:maintenance:hotfix` — **이 문서.** 최소 ceremony. 영향도 분석, RCA, convention 충돌 체크 등은 전부 생략.
> - `/hb-aos:maintenance:auto` — 일상 유지보수. RCA 포함.
> - `/hb-aos:maintenance:deep` — full ceremony, 3방향 영향 Team 포함.
...
예: `BUCCL-AOS-99` 또는 `hotfix-20260408-cookie-nullref`
...
3. 사용자가 제시한 증상을 **FAIL로 입증하는 최소 테스트**를 작성한다.
   - 파일: `app/src/test/kotlin/com/buccl/bucclapp/{package}/{Module}Hotfix{Identifier}Test.kt` (identifier는 CamelCase로 변환)
...
4. `commands/shared/tdd.md`의 **T0 Test Design Check**로 완료기준·행동 assertion·mock 경계를 확인하고 `tdd-design-metadata.json`을 준비한다. 부모 CLI로 선택한 재현 테스트를 실행한다. `--test-file`에는 3번에서 만든 파일의 저장소 상대 경로를 넣는다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check red --repo <root> --test-file {test-file} --cmd './gradlew testDebugUnitTest --tests "*{Module}Hotfix{Identifier}*"' --design {artifacts-dir}/tdd-design-metadata.json --out <root>/.harness/artifacts/maintenance/{identifier} --issue-type hotfix
   ```
...
1. **Red 테스트(H1)가 PASS가 되는 '최소 수정'만** 수행한다. 사용자가 지정한 파일·라인 이외는 수정하지 않는다.
   ...
   - 금지 범주에 해당하면 즉시 중단하고 `/hb-aos:maintenance:auto`로 에스컬레이션한다.
...
1. `./gradlew testDebugUnitTest --tests "*{Module}*"`
2. 필요 시 `./gradlew lint`를 실행한다. lint 수정이 단순 포맷을 넘어가면 `:auto`로 전환한다.
3. 결과 확인:
   - H1 재현 테스트: **PASS**여야 함
...
> 코드 정리가 필요하면 `/hb-aos:maintenance:auto`로 전환한다.
...
- **형제 플랫폼(iOS) 반영 필요 여부** (한 줄 — hotfix에서 계약을 바꿨다면 그 자체가 에스컬레이션 신호)
...
| 상황 | 전환 대상 |
|---|---|
| 수정 범위가 2개 이상 모듈에 걸친다 | `/hb-aos:maintenance:auto` |
| 기존 ADR/convention 위반이 의심된다 | `/hb-aos:maintenance:deep` |
| 회귀가 단위 테스트 범위를 벗어난다 | `/hb-aos:maintenance:auto` (전체 회귀 필요) |
| 새 설계 결정이 필요하다 | `/hb-aos:planning:auto` 또는 `:deep` |
| H2 수정이 2회 연속 실패 | `/hb-aos:maintenance:auto` (RCA 필요) |
| 쿠키/세션 동기화·딥링크·푸시 흐름 변경이 필요하다 | `/hb-aos:maintenance:deep` (shell 계약 흐름은 hotfix 범위 아님) |
| 브리지 계약(함수 시그니처·메시지 포맷) 변경이 필요하다 | `/hb-aos:maintenance:auto` (브리지 계약 변경은 hotfix 범위 아님 — 형제 플랫폼 반영 기록 필요) |
| 리팩토링이 필요하다 | `/hb-aos:maintenance:auto` (hotfix에서는 Refactor 금지) |
