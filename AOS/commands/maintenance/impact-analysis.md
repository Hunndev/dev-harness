...
- `team_name`: `maint-impact-{identifier}` (예: `maint-impact-BUCCL-AOS-42`)
...
network → bridge → webview → Activity → manifest 순으로:
1. 이 원인이 다른 브리지 함수 또는 쿠키/세션 동기화 판단에 영향을 미치는가?
2. 같은 WebView 설정/헬퍼를 쓰는 다른 화면(Activity/Fragment)이 있는가?
...
3. 딥링크 intent-filter 또는 푸시 핸들러에서 호출되는 경우
...
5. 브리지 계약 변경으로 웹 호출부 보정·형제 플랫폼(iOS) 반영이 필요한지 여부
...
- 브리지 계약 변경 → 형제 플랫폼(iOS) 반영 필요: {있음|없음}
