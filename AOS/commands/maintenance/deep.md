...
- 에러 로그(logcat), 재현 절차, 스크린샷, 재현 기기/OS 정보 등이 있으면 제공한다.

## 식별자

이슈 ID가 있으면 그대로 사용. 없으면 `maint-YYYYMMDD-slug` 형식.
예: `BUCCL-AOS-42` 또는 `maint-20260406-cookie-sync-reset`
...
3. 이슈 유형을 분류한다:
   ...
   - `performance` — 느린 WebView 로드, ANR, 앱 크기, 배터리·메모리 누수
   - `dependency` — Gradle 의존성 업그레이드, 보안 패치
   - `device` — 기기/OS 버전별 동작 차이, 푸시 미수신, 딥링크 오동작, 권한 흐름 깨짐 (회귀 검증 시 `device-regression.md`에 전후 확인 기록·재발 방지 노트를 남긴다)
4. `.harness/docs/module-registry.yaml`을 읽고, 관련 모듈(webview/bridge/network/fcm/utils 등)을 식별한다.
5. 사용자에게 다음을 확인한다:
   - 이슈 유형
   - 관련 모듈/레이어
   - 긴급도 (hotfix 여부)
...
2. 재현 테스트 케이스를 작성한다:
   ...
   - JUnit + target repo의 기존 fake/mock 패턴 활용 (Android 프레임워크 의존은 인터페이스 fake로 격리)
...
2. sub-agent에게 전달:
   - `reproduction.md`
   - 관련 모듈 코드 (webview/, bridge/, network/, fcm/, AndroidManifest.xml)
   - `.harness/docs/adr.yaml`
   - `.harness/docs/architecture.yaml`
3. sub-agent는 tracer 스타일로 분석:
   - stack trace(logcat)에서 발생 지점 특정
   - request flow를 따라 문제 지점 추적 (intent/딥링크 → Activity → WebView(BucclWebViewClient) → bridge(WebAppInterface) → network(쿠키 동기화) → 웹(FE))
   - 푸시 흐름이면: FCM 수신 → Service → Notification → 딥링크/화면 진입
...
#### Agent A: 모듈/레이어 방향
```
다음 근본 원인을 기준으로, 수직 방향(network → bridge → webview → Activity → manifest)으로
영향받는 코드를 추적하라.

분석:
1. 이 원인이 다른 브리지 함수 또는 쿠키/세션 동기화 판단에 영향을 미치는가?
2. 이 원인과 같은 WebView 설정/헬퍼를 쓰는 다른 화면(Activity/Fragment)이 있는가?
3. 이 원인이 푸시 수신 경로 또는 딥링크 진입 흐름에도 전파되는가?
...
2. 웹(FE)에서 이 브리지 함수를 호출하는 경로가 있는가? (bridge-contract.yaml 대조)
3. 딥링크 intent-filter 또는 푸시 핸들러에서 호출되는 경우가 있는가?
4. Activity 생명주기, WebViewClient 콜백, Service 중 어디에 끼어 있는가?
...
분석:
1. 이 원인이 쿠키/세션 데이터의 일관성에 영향을 미치는가?
2. 이미 잘못된 데이터가 쌓여 있을 가능성은? (stale 쿠키·토큰·FCM 토큰)
3. SharedPreferences/앱 내부 저장소/WebView 스토리지에 stale 데이터가 있을 가능성은?
4. 메인 BE(hb-be) 또는 웹(FE)에 잘못된 데이터가 전달되었을 가능성은?
5. 브리지 계약 변경으로 웹 쪽 호출부 보정이 필요한가? 형제 플랫폼(iOS) 반영이 필요한가?
...
3. sub-agent는 다음을 확인:
   ...
   - 수정 방향이 convention 규칙(BRIDGE-, WEBVIEW-, NET-, PUSH-, PERM-, TEST- 등)을 위반하는가?
...
2. 계획에 포함:
   ...
   - 범위 제한 (이번에 하지 않는 것)
   - 브리지 계약 또는 매니페스트(권한·intent-filter) 변경 필요 여부
   - 메인 BE·웹(FE) 연동 변경 필요 여부, 형제 플랫폼(iOS) 반영 필요 여부
...
2. 수정 원칙:
   ...
   - **타입 안정성**: 불필요한 `!!`/`@Suppress` 남발 금지, `./gradlew assembleDebug` 통과
...
1. **Claude Code 네이티브 Teams**로 3개 검증을 동시 실행한다. 표준 절차는 `commands/shared/team-protocol.md` 참조.
   ...
   **팀 스펙:**
   ...
   - 팀원 3명 (`subagent_type: general-purpose`, 병렬 스폰):
     ...
     | `build-runner` | 아래 "Agent C" | `.harness/artifacts/maintenance/{identifier}/regression-build.md` |
...
```
다음 검증을 수행하라:
1. ./gradlew assembleDebug (빌드 검사)
2. ./gradlew lint (Android Lint)
3. 매니페스트(권한·intent-filter) 변경이 있으면 diff를 확인
...
2. **메인이 3개 부분 산출물을 병합**:
   - `regression-unit.md`, `regression-e2e.md`, `regression-build.md` Read
   - 전체 통과/실패 요약
   - 새로 발생한 실패 (회귀) 식별
