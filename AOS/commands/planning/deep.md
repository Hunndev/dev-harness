...
예: `plan-20260406-push-deeplink-redesign`
...
2. 다음을 사용자와 함께 확정한다:
   ...
   - 이해관계자 맵 (누가 이 결정에 영향을 받는가)
     ...
     - 시스템 유형별 (AOS 앱 자체, 웹(FE), 메인 BE, 형제 플랫폼 iOS 앱, 푸시 인프라, 스토어)
   - **모호한 논의점** 제시
...
2. 질문 프레임:
   ...
   - "웹(FE) 또는 메인 BE(hb-be)와 어떻게 연결되어야 하는가? WebView/브리지 API 변경이 필요한가?"
   - "푸시·딥링크 요구사항은? 형제 플랫폼(iOS)과 동일해야 하는가?"
   - "성능/규모 요구사항은?" (동시 사용자, 푸시 발송량, 앱 크기/배터리 영향)
...
3. sub-agent는 다음을 조사한다:
   - 유사 서비스/기능의 구현 사례 (Android WebView 앱/하이브리드 shell/푸시/딥링크)
   - 관련 Android 라이브러리, WebView·FCM·딥링크·권한 패턴
   - 알려진 함정이나 안티패턴 (JS 인터페이스 과다 노출, 백그라운드 실행 제한, ANR, 스토어 심사 리젝 사유)
   - BUCCL AOS 스택(Kotlin + Gradle KTS + WebView + FCM)에서의 구현 가능성
...
분석 항목:
1. 각 대안의 구현 방법 (Kotlin 기준, activity/webview/bridge/network/fcm 레이어 수준)
2. 기존 모듈(module-registry.yaml)과의 호환성
3. 기술적 위험 요소와 복잡도 (ANR, 메모리, 동시성, 앱 크기/배터리 영향)
4. 새로운 Gradle 의존성 추가 필요 여부
5. WebView/브리지 API 변경 필요 여부 (형제 플랫폼(iOS) 반영 포함)
6. 푸시·딥링크 구조 변경 또는 매니페스트(권한·intent-filter) 변경 필요 여부
...
3. 기존 사용 흐름과의 일관성
4. 학습 비용과 전환 비용
5. 웹(FE)·형제 플랫폼(iOS)과의 경험 일관성 변화
...
2. 비용 변동 — 스토어 수수료·FCM 비용은 미미하다. 주 비용은 스토어 심사 리드타임과 릴리즈 사이클이다.
3. 웹(FE)·메인 BE(hb-be)와의 연동 변경 비용 (별도 레포 작업 필요)
4. 각 대안의 실패 리스크와 되돌리기 비용 (스토어 배포는 즉시 롤백 불가 — 심사 재대기)
5. 점진적 출시 가능 여부 (staged rollout, 단계적 공개)
6. 스토어 심사·릴리즈 리스크 (정책 위반 소지, 권한 추가 심사)
...
2. `/hb-aos:shared:update-docs adr`을 호출하여 `decision-draft.md` 내용을 `.harness/docs/adr.yaml`에 편입한다.
...
- 결정 사항 요약
- **형제 플랫폼(iOS) 반영 필요 여부** (브리지·푸시·딥링크 계약 방향을 결정했으면 명시)
