...
decision-draft.md는 반드시 사용자 명시 승인 후 `/hb-aos:shared:update-docs adr`로 흘려보낸다.
...
- 나쁜 예: "푸시 알림 정리가 필요해서"
- 좋은 예: "푸시 알림 클릭 시 딥링크가 로그인 화면으로 떨어지는 문의가 반복됐다. 원인은 알림 페이로드의 목적지 URL을 WebView 로드 전에 소비해 쿠키 동기화가 끝나기 전 진입하는 것. 알림 → 딥링크 → 세션 복원 순서를 명문화하지 않으면 형제 플랫폼(iOS)과도 동작이 갈라진다."
...
> 이 문서는 adr.yaml 후보입니다. 편입하려면 사용자 승인 후 `/hb-aos:shared:update-docs adr`을 실행하세요.
...
- bridge-contract.yaml: {변경 필요 / 불필요} — 변경 필요면 형제 플랫폼(iOS) 반영 필요 여부 명시
```
