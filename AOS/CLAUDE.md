...
작업 시작 시 repository의 `AGENTS.md`/`CLAUDE.md`/`.harness/README.md`에서 주제별 소유권을 확인한다. BUCCL AOS가 canonical로 선언한 `.harness/docs/*.yaml`은 코딩 규칙·ADR·architecture·모듈·브리지 계약의 진실의 원천이다. CI·manifest·source와 충돌하면 YAML을 자동으로 무시하지 않고 `BLOCKED` 후 코드 위반인지 문서 drift인지 판정한다.
