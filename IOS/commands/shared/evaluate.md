...
3. 현재 repository snapshot을 재계산해 packet과 다르면 이전 결과를 사용하지 않는다. 재계산은 packet 검증 시점과 source 복사 직후 두 번이며(검증→복사 사이 재검증), 두 번째는 복사본의 tree 해시와도 대조한다 — 사이에 바뀐 뒤 원복된 source도 여기서 걸린다.
