...
프레임워크: **Swift + XCTest + Xcode** (`xcodebuild -scheme bucclapp test`)
...
- exit ≠ 0 → 즉시 중단하고 사용자에게 보고:
  ...
  - 실제 에러 메시지 그대로 전달
- target test 실행은 `-only-testing:bucclappTests/{TestClass}` 식별자(`Target/Class[/Method]`)로 지정한다. 와일드카드·부분 일치는 없다. 존재하지 않는 식별자를 주어도 xcodebuild는 실패하지 않고 0개 실행(XCTest는 `Executed 0 tests`, Swift Testing은 `Test run` 줄 없음)으로 조용히 통과하므로(exit 0), 파일 생성 직후 `-enumerate-tests` 목록에 클래스가 보이는지 확인하고, 부모 `tdd-check`가 리포트에서 실행 수 ≥ 1을 관측해야 한다(아래 "`-only-testing:` 타깃 실행" 절 참조).
...
3. 아래 Test Design 항목을 검수하고 `tdd-design-metadata.json` 입력을 준비한 뒤 부모 CLI로 baseline을 직접 실행한다. `--test-file`은 실제 테스트 파일의 저장소 상대 경로다. `--issue-type`에는 실제 작업 유형(`feature`, `bug`, `performance`, `refactor`, `hotfix`)을 넣는다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check red --repo <root> --test-file {test-file} --cmd 'xcodebuild -project bucclapp/bucclapp.xcodeproj -scheme bucclapp test -only-testing:bucclappTests/{TestClass}' --design {artifacts-dir}/tdd-design-metadata.json --out {artifacts-dir} --issue-type {issue-type} && \
   cp {artifacts-dir}/tdd-baseline-log.txt {artifacts-dir}/xcodebuild-red.log && \
   tail -30 {artifacts-dir}/xcodebuild-red.log > {artifacts-dir}/tdd-baseline-log.txt
   ```

   위 `&&` 후처리는 부모 CLI가 baseline을 수용해 exit 0으로 끝난 경우에만 실행한다. 부모가 기록한 전체 redacted 출력을 `xcodebuild-red.log`에 먼저 보존한 뒤 `tdd-baseline-log.txt`를 tail 30줄로 만든다. 부모 CLI가 실패하면 복사·tail을 수행하지 않고 남은 로그를 진단용으로만 사용하며 Green으로 진행하지 않는다. 실제 xcodebuild의 Red 실패 exit와 부모 CLI의 수용 여부를 구분한다.
...
4. **실패 이유 검증 (bug/feature 유형)**:
   ...
   - Swift 컴파일 에러 / import 에러 / mock 설정 오류 → **올바르지 않은 Red**
...
2. 이름과 assertion이 내부 호출이 아니라 관찰 가능한 행동·상태·부작용·오류 계약을 검증하는가.
3. 필요한 성공·실패·경계·권한 경로가 포함됐는가.
4. 외부 경계만 mock하며 **System Under Test 자체를 mock하지 않았는가**.
5. 기기·시간·비동기 동작에 불필요하게 의존하는 flaky 구조가 아닌가.
6. T2/high-risk는 구현 context와 분리된 read-only lens가 테스트 설계를 독립 확인했는가.
...
4. 관련 회귀·위험도 검수를 마치고 `tdd-sensitivity-metadata.json`을 준비한 뒤 부모 CLI로 변경 후 테스트를 실행한다. `--design`에는 Red에서 생성된 **동일한 고정 Design 파일**을 전달한다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check green --repo <root> --test-file {test-file} --cmd 'xcodebuild -project bucclapp/bucclapp.xcodeproj -scheme bucclapp test -only-testing:bucclappTests/{TestClass}' --design {artifacts-dir}/tdd-test-design-result.json --sensitivity {artifacts-dir}/tdd-sensitivity-metadata.json --out {artifacts-dir} --issue-type {issue-type} && \
   cp {artifacts-dir}/tdd-green-log.txt {artifacts-dir}/xcodebuild-green.log && \
   tail -30 {artifacts-dir}/xcodebuild-green.log > {artifacts-dir}/tdd-green-log.txt
   ```

   부모 CLI가 Green을 수용한 뒤에만 전체 redacted 출력을 `xcodebuild-green.log`에 보존하고 `tdd-green-log.txt`에 tail 30줄을 남긴다. CLI가 실패하면 `&&` 후처리를 실행하지 않는다. 전체 로그와 tail은 진단·표시용이며 봉인된 JSON 관측값을 수정하지 않는다.
...
2. hash 변경은 승인된 Red revision과 새 baseline이 있을 때만 허용한다.
3. repository가 정의한 관련 회귀 suite가 PASS인지 확인한다.
4. 인증·권한·브리지·푸시·딥링크 등 T2/high-risk에서 안전하고 지원되는 경우 격리 worktree의 targeted mutation/revert로 결함이 되살아날 때 테스트가 FAIL하는지 확인한다.
5. 필요한 mutation을 실행할 수 없으면 `BLOCKED` 또는 `NEEDS_HUMAN_REVIEW`로 보낸다.
...
.harness/artifacts/{track}/{identifier}/
  tdd-baseline-log.txt     ← 수용된 Red 전체 로그의 tail 30줄 (FAIL or PASS per issue type)
  tdd-green-log.txt        ← 수용된 Green 전체 로그의 tail 30줄 (refactor 포함)
...
- `tdd-baseline-log.txt`: bug/feature는 FAIL 출력, refactor는 PASS characterization 출력의 tail 30줄. 전체 출력은 `xcodebuild-red.log`에 보존한다.
- `tdd-green-log.txt`: 구현 또는 refactor 후 PASS 출력의 tail 30줄. 전체 출력은 `xcodebuild-green.log`에 보존한다.
...
hotfix 트랙도 부모가 생성한 전체 `hotfix-red-log.txt`, `hotfix-green-log.txt`를 각각 `xcodebuild-red.log`, `xcodebuild-green.log`에 먼저 복사한 뒤 원래 hotfix 파일에 tail 30줄을 남긴다(Refactor 없음). 두 TDD JSON 파일은 hotfix에서도 같은 이름으로 반드시 생성한다. `xcresult` 임시 리포트와 별도 수동 실행 로그는 부모가 봉인한 관측 JSON을 대신하지 않는다.
...
**실행 수 판정 규칙** — `tdd-check`의 판정은 부모가 소유한 `xcresult`에서 읽은 선택된 테스트 ID·실행 수·결과가 정본이다. 아래 로그 요약은 수동 진단에만 사용하며, 리포트 누락이나 0개 실행을 보완하는 증거로 쓰지 않는다.
...
## Swift/iOS Unit Test Harness 카탈로그
...
| 기기 동작 확인 | XCTest(로컬 단위 테스트) 한계 — 에뮬레이터/실기기 `device-check.md` 산출물로 보완 |
