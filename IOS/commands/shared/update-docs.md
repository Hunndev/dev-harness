...
```
/hb-ios:shared:update-docs                — 전체 문서 대상으로 변경 필요 항목 분석
/hb-ios:shared:update-docs convention     — code-convention.yaml만
/hb-ios:shared:update-docs adr            — adr.yaml만
/hb-ios:shared:update-docs architecture   — architecture.yaml만
/hb-ios:shared:update-docs modules        — module-registry.yaml만
/hb-ios:shared:update-docs bridge         — bridge-contract.yaml만
```
...
1. `.harness/docs/module-registry.yaml`의 모듈 목록 vs 실제 소스(`bucclapp/bucclapp/` 하위 패키지 디렉토리 목록)를 대조한다.
...
- id: {카테고리}-{번호}    # GEN, WEBVIEW, BRIDGE, NET, PUSH, PERM, BUILD, TEST, GIT
  rule: ...                 # 명확하고 실행 가능한 규칙
  stacks: [...]             # swift, webview, bridge, fcm, deeplink, permission, all 등
```
...
Info.plist·릴리즈 설정 관련 maintenance 후에는 반드시 갱신한다.
...
```yaml
- name: ...
  paths: [bucclapp/bucclapp/bridge/{...}, bucclapp/bucclapp/webview/{...}, bucclapp/bucclapp/network/{...}, bucclapp/bucclapp/utils/{...}]
  owns: [...]
  checks: [...]
  notes: |
    ...
...
브리지 함수·메시지 포맷·에러 계약의 진실의 원천. **양 플랫폼(ios-buccl ↔ buccl-aos)이 동일 내용을 유지**한다.

```yaml
functions:
  - name: ...                      # 웹이 호출하는 함수명 (WebViewBridge 메서드)
...
2. 갱신 시 형제 레포(buccl-aos)의 동일 파일 반영 필요를 기록하고, `parity-proposal.md`(반영 제안 초안)를 산출물로 남긴다.
3. **buccl-aos 직접 수정 금지 — hb-aos로 전환.** 이 명령은 ios-buccl 레포의 파일만 갱신한다.
4. 현재 `WebViewBridge.swift` 소스와 대조하여 계약-구현 불일치가 발견되면 MUST 항목으로 보고한다.
