...
> **Tier 선택**
> - `/hb-ios:feature:auto` — **이 문서.** 일반 shell 기능/브리지 작업 기본값. deep의 prior-art, 평가기준 재생성, PR본문 Fork를 경량화한다.
> - `/hb-ios:feature:deep` — full ceremony. 브리지 계약, 쿠키/세션 동기화, 푸시·딥링크 구조, Info.plist 권한 영향이 클 때 사용.

## 사전 조건

- `.harness/docs/code-convention.yaml`과 `.harness/docs/adr.yaml`이 작성되어 있어야 한다.
- 요구사항이 확정되어 있어야 한다. 확정되지 않았으면 `/hb-ios:planning:auto` 또는 `:deep`으로.
...
6. **브리지 계약 작업이면** `bridge-check.md`에 기록한다: 함수 시그니처·메시지 포맷·에러 처리의 웹↔네이티브 일치, `.harness/docs/bridge-contract.yaml` 갱신 여부, **형제 플랫폼(AOS) 반영 필요 여부** — 반영이 필요하면 `parity-proposal.md`(buccl-aos용 반영 제안 초안)를 작성한다. (shell 기능 전용 작업이면 "N/A: shell 기능 전용"으로 표기)
...
2. 입력:
   - `design-intent.md` (의도)
   - `device-check.md`, `permission-check.md`, `release-check.md`, `bridge-check.md` (iOS 검증: shell + 브리지)
   - **F4 Red 테스트 파일** (`bucclapp/bucclappTests/{package}/{Module}Feature{Slug}Tests.swift`)
...
3. 리뷰 원칙:
   ...
   - WebView 설정·쿠키 동기화·권한 처리가 ViewController에 흩뿌려지지 않고 담당 계층(webview/network/bridge)을 경유하는지 확인
   - shell 렌즈: 권한 과다, 푸시 수신 경로, 딥링크 충돌, WebView 보안 설정(JS 인터페이스 노출 범위) 확인
   - 브리지 렌즈: 함수 시그니처, 메시지 포맷, 에러 처리, 형제 플랫폼(AOS)과의 계약 동일성 확인
