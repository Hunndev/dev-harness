...
- `xcodebuild -scheme bucclapp build`를 항상 통과시킨다. SwiftLint는 설치·설정된 경우에만 실행하고 미구성이면 N/A로 기록한다(`commands/shared/verify.md` 2. 린트).
- webview/bridge/network/푸시(FCM)/utils 경계를 유지한다.
...
2. 작업 모드를 분류한다: **shell 기능 / 브리지 계약 / 혼합** (CLAUDE.md "iOS 작업 두 모드" 참조).
...
3. sub-agent는 다음을 조사:
   ...
   - 기존 코드와 충돌할 가능성이 있는 부분은? (브리지 함수명 충돌, Universal Link·커스텀 스킴 충돌, 권한 중복, 쿠키 도메인 충돌)
   - 기존 `WebViewBridge`, `WebViewContainer`/`WebViewContainer`, FCM 헬퍼를 어떻게 활용할 것인가?
...
2. `design-intent.md`에 아래 내용을 포함:
   ...
   - 관련 컴포넌트 계층 (ViewController / webview / bridge / network / fcm / Info.plist)
   - 핵심 설계 결정과 트레이드오프 (어떤 로직을 어느 계층에 둘지, 브리지 함수 시그니처, 쿠키 동기화 책임)
   - WebView 설정·권한·푸시·딥링크 영향
   - 브리지 계약 변경 여부 — 있으면 `bridge-contract.yaml` 갱신 계획과 형제 플랫폼(AOS) 반영 필요 여부
   - 의도적으로 제외한 것
...
3. `xcodebuild -scheme bucclapp test -only-testing:bucclappTests/{Module}Feature{Slug}Tests`로 실행하여 FAIL을 확인한다. 로그의 `Executed N tests`가 0이면 식별자 불일치이므로 `-enumerate-tests`로 클래스명을 대조한다.
...
3. 중복 제거, 네이밍 개선, 구조 정리를 수행한다.
   - **새 기능 금지, 테스트 변경 금지**
4. 각 리팩토링 후 `xcodebuild -scheme bucclapp test`로 전체 그린을 확인한다. 깨지면 즉시 revert.
...
6. **브리지 계약 작업이면** `bridge-check.md`에 기록한다: 함수 시그니처·메시지 포맷·에러 처리의 웹↔네이티브 일치, `.harness/docs/bridge-contract.yaml` 갱신 여부, **형제 플랫폼(AOS) 반영 필요 여부** — 반영이 필요하면 `parity-proposal.md`(buccl-aos용 반영 제안 초안)를 작성한다. (shell 기능 전용 작업이면 "N/A: shell 기능 전용")
...
3. PR 본문 구조:
   - Summary (1-3문장)
   - Changes (모듈 단위: bridge/webview/network/푸시(FCM)/Info.plist/utils 등)
   - Breaking Changes
   - Test Plan (XCTest 테스트 + 기기/권한/릴리즈 확인)
   - 브리지 계약/Info.plist 변경 (형제 플랫폼(AOS) 반영 필요 여부 포함)
...
2. 입력:
   ...
   - `pr-body.md`
   - **F6 Red 테스트 파일** (`bucclapp/bucclappTests/{package}/{Module}Feature{Slug}Tests.swift`)
...
3. 리뷰 원칙:
   ...
   - 브리지 계약 시: 함수 시그니처·메시지 포맷 일치, 에러 처리 누락, 형제 플랫폼(AOS)과의 계약 동일성, WebViewBridge 단일 계층 우회 확인
...
4. QA 수행:
   ...
   - `xcodebuild -scheme bucclapp test -only-testing:bucclappTests/{Module}Feature{Slug}Tests` (관련 테스트 클래스 — `Executed N tests`의 N ≥ 1 확인)
...
- 산출물 목록
- 생성/변경된 파일 목록
- 브리지 계약/Info.plist 변경 내역 목록 (있으면)
- 테스트 결과 요약
- **형제 플랫폼(AOS) 반영 필요 여부** (필요하면 `parity-proposal.md` 경로 명시)
- 커밋 메시지 제안
