...
> **Tier 선택**
> - `/hb-ios:maintenance:hotfix` — 오타, 한 줄 fix, 긴급 수정. 재현 테스트 + 수정 + 단위 테스트 3단계.
> - `/hb-ios:maintenance:auto` — **이 문서.** 일상 유지보수 기본값. M4 영향도 3방향 Team, M5 ADR 충돌 체크 없음.
> - `/hb-ios:maintenance:deep` — M1~M9 full ceremony. 모듈 경계가 불분명하거나 기존 ADR 위반이 의심될 때.
...
예: `BUCCL-iOS-42` 또는 `maint-20260408-push-token-refresh`

## 핵심 원칙

- 회귀 방지 최우선
- 수정 범위 폭주 방지 → `fix-plan.md`에서 범위 명시
- **신규 ADR 생성 금지** — 새 결정이 필요하면 `/hb-ios:planning:auto` 또는 `:deep`으로 에스컬레이션
...
3. 이슈 유형을 분류한다:
   ...
   - `dependency` — Xcode 의존성 업그레이드, 보안 패치
   - `device` — 기기/OS 버전별 동작 차이, 푸시 미수신, 딥링크 오동작, 권한 흐름 깨짐
4. `.harness/docs/module-registry.yaml`을 읽고, 관련 webview/bridge/network/푸시(FCM)/utils 모듈을 식별한다.
5. 사용자에게 다음을 확인한다:
   - 이슈 유형
   - 관련 모듈
...
2. sub-agent에게 전달:
   - `reproduction.md`
   - 관련 모듈 코드 (ViewController, webview, bridge, network, fcm, Info.plist)
...
2. 계획에 포함:
   ...
   - **범위 제한** (이번에 하지 않는 것)
   - 브리지 계약/Info.plist(권한·Universal Link·커스텀 스킴) 변경 필요 여부 — 있으면 형제 플랫폼(AOS) 반영 필요 여부
...
2. 수정 원칙:
   ...
   - **convention 준수**: 브리지 단일 계층(WebViewBridge), webview/network/fcm 책임 경계, 명백한 규칙 위반 즉시 체크
...
2. 모든 테스트가 PASS인지 먼저 확인 (`xcodebuild -scheme bucclapp test`).
...
> fix-plan 범위를 벗어나는 리팩토링 유혹이 생기면 `/hb-ios:maintenance:deep`으로 에스컬레이션하거나 별도 maintenance 작업으로 분리한다.
...
3. 전체 테스트: `xcodebuild -scheme bucclapp test`
   - 새로 실패한 테스트가 있는지 확인
4. build 검증: `xcodebuild -scheme bucclapp build`
