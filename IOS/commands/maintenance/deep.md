...
예: `BUCCL-iOS-42` 또는 `maint-20260406-cookie-sync-reset`
...
4. `.harness/docs/module-registry.yaml`을 읽고, 관련 모듈(webview/bridge/network/푸시(FCM)/utils 등)을 식별한다.
5. 사용자에게 다음을 확인한다:
   - 이슈 유형
   - 관련 모듈/레이어
   - 긴급도 (hotfix 여부)
...
2. sub-agent에게 전달:
   - `reproduction.md`
   - 관련 모듈 코드 (webview/, bridge/, network/, 푸시(FCM), Info.plist)
   - `.harness/docs/adr.yaml`
   - `.harness/docs/architecture.yaml`
3. sub-agent는 tracer 스타일로 분석:
   - stack trace(logcat)에서 발생 지점 특정
   - request flow를 따라 문제 지점 추적 (URL/딥링크 → ViewController → WebView(WebViewContainer) → bridge(WebViewBridge) → network(쿠키 동기화) → 웹(FE))
...
#### Agent A: 모듈/레이어 방향
```
다음 근본 원인을 기준으로, 수직 방향(network → bridge → webview → ViewController → Info.plist)으로
영향받는 코드를 추적하라.

분석:
1. 이 원인이 다른 브리지 함수 또는 쿠키/세션 동기화 판단에 영향을 미치는가?
2. 이 원인과 같은 WebView 설정/헬퍼를 쓰는 다른 화면(ViewController/View)이 있는가?
...
3. 딥링크 Universal Link·커스텀 스킴 또는 푸시 핸들러에서 호출되는 경우가 있는가?
...
5. 브리지 계약 변경으로 웹 쪽 호출부 보정이 필요한가? 형제 플랫폼(AOS) 반영이 필요한가?
...
2. 계획에 포함:
   ...
   - 범위 제한 (이번에 하지 않는 것)
   - 브리지 계약 또는 Info.plist(권한·Universal Link·커스텀 스킴) 변경 필요 여부
   - 메인 BE·웹(FE) 연동 변경 필요 여부, 형제 플랫폼(AOS) 반영 필요 여부
