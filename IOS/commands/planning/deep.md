...
2. 다음을 사용자와 함께 확정한다:
   ...
   - 이해관계자 맵 (누가 이 결정에 영향을 받는가)
     ...
     - 시스템 유형별 (iOS 앱 자체, 웹(FE), 메인 BE, 형제 플랫폼 AOS 앱, 푸시 인프라, 스토어)
   - **모호한 논의점** 제시
...
2. 질문 프레임:
   ...
   - "푸시·딥링크 요구사항은? 형제 플랫폼(AOS)과 동일해야 하는가?"
...
3. sub-agent는 다음을 조사한다:
   - 유사 서비스/기능의 구현 사례 (iOS WebView 앱/하이브리드 shell/푸시/딥링크)
   - 관련 iOS 라이브러리, WebView·FCM·딥링크·권한 패턴
   - 알려진 함정이나 안티패턴 (JS 인터페이스 과다 노출, 백그라운드 실행 제한, ANR, 스토어 심사 리젝 사유)
   - BUCCL iOS 스택(Swift + Xcode + WebView + FCM)에서의 구현 가능성
...
5. WebView/브리지 API 변경 필요 여부 (형제 플랫폼(AOS) 반영 포함)
6. 푸시·딥링크 구조 변경 또는 Info.plist(권한·Universal Link·커스텀 스킴) 변경 필요 여부
...
3. 기존 사용 흐름과의 일관성
4. 학습 비용과 전환 비용
5. 웹(FE)·형제 플랫폼(AOS)과의 경험 일관성 변화
...
2. `/hb-ios:shared:update-docs adr`을 호출하여 `decision-draft.md` 내용을 `.harness/docs/adr.yaml`에 편입한다.
...
- 결정 사항 요약
- **형제 플랫폼(AOS) 반영 필요 여부** (브리지·푸시·딥링크 계약 방향을 결정했으면 명시)
