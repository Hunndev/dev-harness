...
1. `.harness/docs/module-registry.yaml`의 모듈 목록 vs 실제 소스(Django 앱 디렉토리(최상위 `*/apps.py` 보유 디렉토리))를 대조한다.
...
- 좋은 예: "Docker Swarm 환경에서 MySQL 연결이 wait_timeout 이후 끊어지면서 'MySQL server has gone away' 에러가 간헐적으로 발생했다. Django 기본값(CONN_MAX_AGE=0)은 매 요청마다 연결을 새로 맺어 성능 저하가 있었다."
