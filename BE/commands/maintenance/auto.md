...
> **Tier 선택**
> - `/hb-be:maintenance:hotfix` — 오타, 한 줄 fix, 긴급 수정. 재현 테스트 + 수정 + 단위 테스트 3단계.
> - `/hb-be:maintenance:auto` — **이 문서.** 일상 유지보수 기본값. M4 영향도 3방향 Team, M5 ADR 충돌 체크 없음.
> - `/hb-be:maintenance:deep` — M1~M9 full ceremony. 모듈 경계가 불분명하거나 기존 ADR 위반이 의심될 때.
...
예: `BUCCL-42` 또는 `maint-20260408-conn-reset`

## 핵심 원칙

- 회귀 방지 최우선
- 수정 범위 폭주 방지 → `fix-plan.md`에서 범위 명시
- **신규 ADR 생성 금지** — 새 결정이 필요하면 `/hb-be:planning:auto` 또는 `:deep`으로 에스컬레이션
...
4. `.harness/docs/module-registry.yaml`을 읽고, 관련 모듈을 식별한다.
5. 사용자에게 다음을 확인한다:
   - 이슈 유형
   - 관련 모듈
...
3. Baseline 로그를 `.harness/artifacts/maintenance/{identifier}/tdd-baseline-log.txt`에 저장한다:
   ...
   - performance 유형: 기준선(응답시간, 쿼리 수, 메모리)을 기록.
...
5. **본문 제시 (필수)**: 분석 결과를 사용자에게 제시할 때 단순 요약이 아니라 `root-cause.md` 의 핵심 (traceback 발생 지점 / 원인 추정 순위 / 근거) 을 마크다운으로 직접 보여준다.
...
2. 수정 원칙:
   ...
   - **convention 준수**: 명백한 규칙 위반은 즉시 체크
...
2. 모든 테스트가 PASS인지 먼저 확인 (`pytest {app}/tests/ -v`).
...
> fix-plan 범위를 벗어나는 리팩토링 유혹이 생기면 `/hb-be:maintenance:deep`으로 에스컬레이션하거나 별도 maintenance 작업으로 분리한다.
...
- 외부 서비스(Innopay, Azure Blob)에 영향이 전파되었을 가능성
