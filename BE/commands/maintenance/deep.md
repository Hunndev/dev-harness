...
5. **본문 제시 (필수)**: 분석 결과를 사용자에게 제시할 때 단순 요약이 아니라 `root-cause.md` 의 핵심 (traceback 발생 지점 / 원인 추정 순위 / 근거 / 관련 ADR 충돌 여부) 을 마크다운으로 직접 보여준다.
...
4. 팀 해체: 팀원들에게 `shutdown_request` → `TeamDelete`.
...
1. **Claude Code 네이티브 Teams**로 3개 검증을 동시 실행한다. 표준 절차는 `commands/shared/team-protocol.md` 참조.
   ...
   **팀 스펙:**
   ...
   - 팀원 3명 (`subagent_type: general-purpose`, 병렬 스폰):
     ...
     | `lint-runner`  | 아래 "Agent C" | `.harness/artifacts/maintenance/{identifier}/regression-lint.md` |
...
2. **메인이 3개 부분 산출물을 병합**:
   - `regression-unit.md`, `regression-e2e.md`, `regression-lint.md` Read
   - 전체 통과/실패 요약
   - 새로 발생한 실패 (회귀) 식별
