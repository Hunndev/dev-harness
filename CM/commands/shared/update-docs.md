...
1. `.harness/docs/module-registry.yaml`의 모듈 목록 vs 실제 소스(`src/routes/`·`src/services/` 파일 목록)를 대조한다.
...
```yaml
- id: {카테고리}-{번호}    # GEN, CM, TS, EXPRESS, SQL, TEST, GIT
  rule: ...                 # 명확하고 실행 가능한 규칙
  stacks: [...]             # node, ts, express, mysql, redis, all 등
```
