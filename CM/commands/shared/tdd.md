...
```bash
npx jest --listTests --silent
npx jest --version
```

- exit 0 → Jest 설치됨, `jest.config.{js,ts}` 정상 → 사이클 진행
...
3. 아래 Test Design 항목을 검수하고 `tdd-design-metadata.json` 입력을 준비한 뒤 부모 CLI로 baseline을 직접 실행한다. `--test-file`은 실제 테스트 파일의 저장소 상대 경로다. `--issue-type`에는 실제 작업 유형(`feature`, `bug`, `performance`, `refactor`, `hotfix`)을 넣는다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check red --repo <root> --test-file {test-file} --cmd 'npm test -- --testPathPattern={module}' --design {artifacts-dir}/tdd-design-metadata.json --out {artifacts-dir} --issue-type {issue-type}
   ```
...
4. **실패 이유 검증 (bug/feature 유형)**:
   ...
   - TypeScript 컴파일 에러 / import 에러 / mock 설정 오류 → **올바르지 않은 Red**
...
4. 관련 회귀·위험도 검수를 마치고 `tdd-sensitivity-metadata.json`을 준비한 뒤 부모 CLI로 변경 후 테스트를 실행한다. `--design`에는 Red에서 생성된 **동일한 고정 Design 파일**을 전달한다.
   ...
   ```bash
   <플러그인 설치 경로>/bin/hb-eval-review tdd-check green --repo <root> --test-file {test-file} --cmd 'npm test -- --testPathPattern={module}' --design {artifacts-dir}/tdd-test-design-result.json --sensitivity {artifacts-dir}/tdd-sensitivity-metadata.json --out {artifacts-dir} --issue-type {issue-type}
   ```
...
- TypeScript strict 모드 위반 (`any` 남발, `@ts-ignore`로 타입 오류 회피)
...
## Node/Express/Socket.io Test Harness 카탈로그
...
| 영역 | 해결책 |
|------|------|
| Express 엔드포인트 | `supertest(app)` 사용, `.get()/.post()` 체이닝 |
| Express 미들웨어 체인 | `supertest` + middleware 순서 조립한 테스트용 `express()` 앱 |
| Socket.io 이벤트 | `socket.io-client`로 실제 연결, 또는 `jest.fn()`으로 mock emitter |
| Socket.io 인증 미들웨어 | 테스트용 JWT 토큰 생성 헬퍼 + `auth: { token }` 옵션 |
| Redis 캐시 | `ioredis-mock` 라이브러리 또는 testcontainers-redis |
| Redis pub/sub | `ioredis-mock`의 pub/sub 지원 또는 직접 EventEmitter mock |
| MySQL repository | testcontainers-mysql 또는 트랜잭션 롤백 기반 테스트 DB |
| JWT 검증 | 테스트 전용 secret + `jsonwebtoken.sign()` 헬퍼 |
| External HTTP (메인 BE 호출) | `nock` 또는 `msw`로 mock HTTP 서버 |
| EventLoop blocking 감지 | `clinic.js` 또는 `autocannon` — 성능 테스트 유형에서만 |
| DB connection pool 검증 | `testcontainers-mysql` + pool stats inspection |
