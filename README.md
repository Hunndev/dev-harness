...
`FE/` 플러그인은 Codex용 `.codex-plugin/plugin.json`과 `skills/hb-fe/SKILL.md`도 포함한다.
repo-local Codex marketplace는 `.agents/plugins/marketplace.json`에 있으며 `./FE` 플러그인을 가리킨다.
Codex는 Claude slash command를 직접 실행하지 않으므로, `hb-fe feature auto로 이 화면 구현해줘`처럼 자연어 alias로 사용한다.
Codex skill은 같은 `FE/commands/` 문서를 source of truth로 읽고 동일한 `.harness/artifacts/` 산출물 규약을 따른다.
...
├── .agents/plugins/
│   └── marketplace.json          ← Codex용 hb-fe 등록
├── BE/                           ← Django 플러그인
...
├── scripts/lint-harness.sh       ← R1~R8 린터
└── README.md
```
