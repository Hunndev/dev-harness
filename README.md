...
세 플러그인(`BE/`, `CM/`, `FE/`) 모두 Codex용 `.codex-plugin/plugin.json`과 `skills/<plugin>/SKILL.md`를 포함한다.
repo-local Codex marketplace는 `.agents/plugins/marketplace.json`에 있으며 `./BE`, `./CM`, `./FE` 세 플러그인을 모두 가리킨다.
