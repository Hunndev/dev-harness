...
실제 표 바로 뒤의 들여쓴 `<!--`는 문단 연속 주석으로 스캔하지 않고 코드 구간으로 숨긴다. 이 구간은 주석의 `-->`가 있는 줄까지, 또는 비어 있지 않은 줄이 목록 컨테이너+4칸보다 얕아지는 지점의 직전까지 중 먼저 오는 지점에서 끝난다. 한 줄에서 닫힌 주석은 그 다음 줄부터 기존 기준 수집을 재개한다. 닫히지 않은 주석은 들여쓰기 감소 전까지 코드로 유지하는 현재 동작을 의도적으로 고정한다(사용자 결정 (b)). 기준이 하나도 남지 않으면 기존 packet 검증이 요청을 차단한다.
